...
\section{Bayesian Modelling for Binary Response Data}

Bayesian inference provides a probabilistic framework for estimating unknown model parameters by combining information from the likelihood with prior information. For a parameter vector $\boldsymbol{\beta}$ and observed data $(\mathbf{y},\mathbf{X})$, the posterior distribution is proportional to
\[
\pi(\boldsymbol{\beta}\mid \mathbf{y},\mathbf{X})
\propto
L(\mathbf{y}\mid \boldsymbol{\beta},\mathbf{X})\pi(\boldsymbol{\beta}).
\]
The posterior distribution is therefore the central object of inference because it describes uncertainty about the regression coefficients after observing the data. In this study, posterior inference is required for a binary accident-severity response, so Bayesian logistic regression provides the modelling foundation for the subsequent Metropolis--Hastings and Consensus Monte Carlo procedures.

Logistic regression is appropriate when the response variable is binary. Let $y_i\in\{0,1\}$ denote the response for observation $i$, where $y_i=1$ represents a serious/fatal accident and $y_i=0$ represents a slight accident. The Bayesian logistic regression model assumes
\[
y_i\mid \boldsymbol{\beta}\sim \operatorname{Bernoulli}(p_i),
\qquad
\operatorname{logit}(p_i)=\mathbf{x}_i^T\boldsymbol{\beta}.
\]
Here, $p_i$ is the probability of serious/fatal accident severity and $\mathbf{x}_i$ contains the intercept and encoded predictor values. Because road type and speed limit are categorical predictors, one category must be treated as the reference category. The coefficients for the remaining categories are then interpreted as changes in log-odds relative to the reference category, holding other predictors fixed. This reference-category interpretation is important because the empirical implementation models road type and speed limit through indicator variables.
