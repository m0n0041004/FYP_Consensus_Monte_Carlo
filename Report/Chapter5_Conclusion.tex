...
Consensus Monte Carlo was used as the main distributed Bayesian approach because the thesis studies posterior simulation for large datasets under a shard-based computational framework. It allows the data to be divided into shards and analysed through shard-level posterior simulation, after which the subposterior samples are combined to approximate the full-data posterior. In the current implementation, the full-data samplers are run sequentially, whereas the CMC shard computations use single-machine, shared-memory parallel workers. The consensus posterior must still be assessed against the full-data posterior rather than assumed to be equivalent or superior.

Posterior credible intervals were used to assess whether coefficients were credibly different from zero. Across the Bayesian summaries in Chapter 4, the credible interval for Road type: one way street (from 2005) contained zero, while the intervals for the other non-intercept coefficients excluded zero. This interpretation is conditional on the fitted model, selected predictors, and posterior simulation output.

The runtime results must be interpreted carefully. Table \ref{tab:acceptance_runtime} reports shorter recorded mean CMC wall-clock runtimes than the corresponding full-data runtimes under the current single-machine parallel setup, so the improvement is an empirical result for this implementation. A multi-machine distributed cluster was not used. The result should not be generalised as hardware-independent or multi-machine distributed speedup because runtime depends on machine resources, communication cost, parallel-backend overhead, posterior-combination cost, implementation details, and statistical efficiency. In particular, using $K$ shards does not imply a $K$-fold speedup.
...
Fourth, the Consensus Monte Carlo shard computations used single-machine, shared-memory parallel workers rather than a multi-machine distributed cluster. The recorded wall-clock runtime therefore reflects the current machine, parallel backend, cluster setup and worker overhead, and posterior-combination procedure.

Fifth, only $K=10$ shards were considered. Different shard counts may affect subposterior approximation, sampler behaviour, posterior combination, and computational cost.
...
Future work could extend this study in several directions. First, the Consensus Monte Carlo procedures could be implemented on a multi-machine distributed cluster. Comparing that environment with the current single-machine parallel implementation would allow further assessment of wall-clock runtime, inter-machine communication cost, and posterior-combination overhead.

Second, future studies could examine values of $K$ other than the $K=10$ configuration used in this thesis. Varying the number of shards would make it possible to study how data partitioning affects posterior approximation quality, acceptance rates, effective sample sizes, Monte Carlo standard errors, and runtime.
...
Sixth, further convergence diagnostics could be added. Additional autocorrelation analysis and posterior predictive checks would provide a broader assessment of posterior simulation quality beyond the multi-chain diagnostics already reported.
